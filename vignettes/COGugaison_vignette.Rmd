...
Ici, nous allons transformer les variables numériques de la table *exemple_pop* afin de récupérer les données de population et de superficie des communes au 1er janvier 2016 (au lieu de 2014).

```{r, fig.show='hold'}
exemple_popcom_COG2016_num <- changement_COG_varNum(table_entree=exemple_popcom,annees=c(2014:2016),agregation=T,libgeo="LIBGEO",donnees_insee=T)
head(exemple_popcom_COG2016_num)
```
...
La fonction `changement_COG_typo` permet de transformer des typologies de communes en géographie au premier janvier d'une année souhaitée. Ici nous allons transformer les deux typologies (typoA et typoB) de la table *exemple_pop* en géographie communale au 1er janvier 2016 (au lieu de 2014).
...
```{r, fig.show='hold'}
exemple_popcom_COG2016_typo <- changement_COG_typo(table_entree=exemple_popcom[,-2],annees=c(2014:2016),methode_fusion="methode_difference",typos=c("typoA","typoB"),mot_difference = "differents",libgeo="LIB",donnees_insee=T)
head(exemple_popcom_COG2016_typo)
```
La fonction `changement_COG_typo_details` permet quant à elle d'isoler dans une table les communes fusionnées appartenant à des classes différentes, ici selon la typologie "typoA" entre 2014 et 2015 puis 2015 et 2016. 

```{r, fig.show='hold'}
details_exemple_popcom_COG2016_typo <- changement_COG_typo_details(table_entree=exemple_popcom[,-2],annees=c(2014:2016),typo="typoA", donnees_insee=T)
head(details_exemple_popcom_COG2016_typo[["2014_2015"]])
head(details_exemple_popcom_COG2016_typo[["2015_2016"]])
```
...
```{r, fig.show='hold'}
exemple_popcom_COG2016 <- merge(exemple_popcom_COG2016_num,exemple_popcom_COG2016_typo[,-2],by="CODGEO",all=T)
exemple_popcom_ZE2010 <- nivsupra(table_entree=exemple_popcom_COG2016,nivsupra="ZE2010",na.nivgeo.rm=F,agregation=T)
...
```{r, fig.show='hold'}
exemple_flux_COG2016 <- changement_COG_varNum(table_entree=exemple_flux_sansPLMsansCorse,annees=c(2014:2016),codgeo_entree="COMMUNE",agregation=F,libgeo=NULL,donnees_insee=T)
exemple_flux_COG2016 <- changement_COG_varNum(table_entree=exemple_flux_COG2016,annees=c(2014:2016),codgeo_entree="DCLT",agregation=F,libgeo=NULL,donnees_insee=T)
```
...
```{r, fig.show='hold'}
exemple_flux_COG2016_etZE <- nivsupra(table_entree=exemple_flux_COG2016,code_com="COMMUNE",nivsupra="ZE2010",na.nivgeo.rm=F,agregation=F)
exemple_flux_COG2016_etZE <- nivsupra(table_entree=exemple_flux_COG2016_etZE,code_com="DCLT",nivsupra="ZE2010",na.nivgeo.rm=F,agregation=F)
head(exemple_flux_COG2016_etZE)
```
