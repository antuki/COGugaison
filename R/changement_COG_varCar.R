#' @title Titre
#' @name changement_COG_varCar
#' @description Remplir
#' @return Remplir
#' @export

changement_COG_varCar <- function(table_entree,annees,codgeo_entree=colnames(table_entree)[1],methode_fusion="methode_difference",mot_different="différents"){
  ...
  for (i in 1:(length(annees)-1)){
    ...
    variables <- colnames(provisoire)[6:length(colnames(provisoire))]
